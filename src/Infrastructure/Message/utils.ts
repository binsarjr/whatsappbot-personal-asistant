...
export const getImageOrVideo = (chat: proto.IWebMessageInfo) => {
    ...
    return (
        ...
        extendedMsg?.imageMessage ||
        extendedMsg?.extendedTextMessage?.contextInfo?.quotedMessage
            ?.imageMessage ||
        msg?.videoMessage ||
        msg?.extendedTextMessage?.contextInfo?.quotedMessage?.videoMessage ||
        extendedMsg?.videoMessage ||
        extendedMsg?.extendedTextMessage?.contextInfo?.quotedMessage
            ?.videoMessage ||
        null
    )
